...
- Forge：26.2-65.1.3 已构建，客户端验证进行中。
- Fabric 1.21.9、1.21.11 已构建；1.21.11 使用新的聊天点击注入点。
...
26.x 已内置 ChatImageCode，并直接维护 `show_chatimage` 的聊天事件适配，不再需要单独安装 ActionLib。旧版依赖仍按上游各版本的元数据执行。
